...
### Added
- **Richer template variables** — `template.md` now supports `{{day_of_week}}` (e.g. Saturday), `{{week_number}}` (ISO week, e.g. 25), and `{{time}}` (HH:MM), in addition to the existing `{{date}}`.

## [5.5.0] - 2026-06-21

### Added
- **fzf-powered `--list`** — when `fzf` is installed, `--list` becomes an interactive fuzzy-finder with full entry preview on the right. Select an entry and press Enter to open it. Falls back to the existing plain list when fzf is not available.

## [5.4.0] - 2026-06-21

### Added
- **`--last` flag** — open the most recent journal entry without knowing its date. Falls through to the standard flow (git sync, editor, commit prompt) just like `--yesterday`.

## [5.3.0] - 2026-06-21

### Added
- **Streak tracking in `--stats`** — current and longest consecutive-day streaks, computed from dated entries only (timestamped entries excluded). Cross-platform via `date_to_epoch` helper (macOS + Linux).
